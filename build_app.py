...
<main>
  ...
  <section id="view-menu">
    ...
    <div class="filter-presets">
      ...
      <button class="chip-btn" onclick="App.applyPreset('trio', this)">⭐ Базовое Трио (18-24б)</button>
      <button class="chip-btn" onclick="App.applyPreset('integrals', this)">📐 Интегралы и замены</button>
      <button class="chip-btn" onclick="App.applyPreset('odes', this)">ОДУ 1-2 порядков</button>
      <button class="chip-btn" onclick="App.applyPreset('pdes', this)">УрЧП и первые интегралы</button>
      <button class="chip-btn" onclick="App.applyPreset('theor', this)">Теорминимум</button>
      <button class="chip-btn" onclick="App.applyPreset('blitz', this)">Блиц в уме</button>
...
const App = {
  ...
  applyPreset(preset, btn) {
    ...
    if (preset === "all") {
      ...
      DATA.SECTIONS.filter(s => s.p === 1).forEach(s => this.selectedSections.add(s.n));
    } else if (preset === "integrals") {
      DATA.SECTIONS.filter(s => s.p === 0).forEach(s => this.selectedSections.add(s.n));
    } else if (preset === "odes") {
      DATA.SECTIONS.filter(s => s.p === 2).forEach(s => this.selectedSections.add(s.n));
    } else if (preset === "pdes") {
      DATA.SECTIONS.filter(s => s.p === 3).forEach(s => this.selectedSections.add(s.n));
    } else if (preset === "theor") {
      DATA.SECTIONS.filter(s => s.p === 4).forEach(s => this.selectedSections.add(s.n));
    } else if (preset === "blitz") {
      DATA.SECTIONS.filter(s => s.p === 5).forEach(s => this.selectedSections.add(s.n));
